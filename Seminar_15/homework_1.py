...
# Добавлено к ниму логирование ошибок и полезной информации.
...
def user_input_number(message: str, lower_limit: int, upper_limit: int) -> int:
    ...
    while True:
        try:
            ...
            logging.error('Неверный Ввод. Введите натуральное число.')
...
def main(lower_limit: int, upper_limit: int, number: int = None):
    ...
    if number is None:
        message_for_user = f'Введите натуральное число от {lower_limit} до {upper_limit}: '
...
if __name__ == '__main__':
    ...
    if args.number:
        if LOWER_LIMIT <= args.number <= UPPER_LIMIT:
            ...
            logging.error(
                f'Число должно находиться между {LOWER_LIMIT} и {UPPER_LIMIT}.')
    else:
